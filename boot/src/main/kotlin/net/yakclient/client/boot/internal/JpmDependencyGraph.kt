...
//                    .all { r ->
//                        fua Configuration.provides(name: String) : Boolean = modules().any { it.name() == name } || parents().any {it.provides(name)}
//
//                        dependants.any { d -> r.name() == d.name || (d as ResolvedJpm).configuration.provides(r.name()) } || ModuleLayer.boot().modules().any { d -> r.name() == d.name }
//                    }) {
//                "A Dependency of ${reference.descriptor().name()} is not in the graph!"
//            }
//
//            val references = dependants.filterIsInstance<ResolvedJpm>().takeIf { it.size == dependants.size }
//                ?: throw IllegalArgumentException("All dependants must be of type ${ResolvedJpm::class.simpleName}")
//
