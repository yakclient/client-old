...
// All credit to Trustin Heuiseung Lee over at os-maven-plugin (https://github.com/trustin/os-maven-plugin)
public class Detector {
    ...
    private Detector(Properties systemProperties) {
        this.systemProperties = systemProperties;
    }

    public synchronized static Detector getInstance() {
        if (INSTANCE == null) INSTANCE = new Detector(System.getProperties());
        return INSTANCE;
    }
...
//    }
    ...
    public Map<String, String> detect(List<String> classifierWithLikes) {
        final Map<String, String> properties = new HashMap<>();

        final String osName = systemProperties.getProperty("os.name");
        final String osArch = systemProperties.getProperty("os.arch");
        final String osVersion = systemProperties.getProperty("os.version");
        ...
        properties.put(DETECTED_NAME, detectedName);
        properties.put(DETECTED_ARCH, detectedArch);
        properties.put(DETECTED_BITNESS,"" + detectedBitness);

        final Matcher versionMatcher = VERSION_REGEX.matcher(osVersion);
        if (versionMatcher.matches()) {
            properties.put(DETECTED_VERSION, versionMatcher.group(1));
            properties.put(DETECTED_VERSION_MAJOR, versionMatcher.group(2));
            properties.put(DETECTED_VERSION_MINOR, versionMatcher.group(3));
        }

//        final String failOnUnknownOS =
//                properties.getProperty("failOnUnknownOS");
...
//        }
        ...
        if (linuxRelease != null) {
            properties.put(DETECTED_RELEASE, linuxRelease.id);
            if (linuxRelease.version != null) {
                properties.put(DETECTED_RELEASE_VERSION, linuxRelease.version);
            }
        ...
        properties.put(DETECTED_CLASSIFIER, detectedClassifierBuilder.toString());
...
//    protected abstract void logProperty(String name, String value);
    ...
    private LinuxRelease parseLinuxRedhatReleaseFile() {
        BufferedReader reader = null;
        try {
            InputStream in = new FileInputStream(Detector.REDHAT_RELEASE_FILE);
    ...
    private int determineBitness(String architecture) {
        ...
        String bitness = systemProperties.getProperty("sun.arch.data.model", "");
        ...
        bitness = systemProperties.getProperty("com.ibm.vm.bitmode", "");
